  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (checkingMaintenance === true) return message.lineReply(`Maintenance mode is on so u can't use the bot right now, Try it again later \:)`);
      ...
      if (!args[0]) {
        ...
        return message.channel.send("Unknown Format Usage! either use `none` / `all` to see **Guilds** && **Users** with permissions, or too see each one: `guilds` / `users`. Usage: `<prefix>premiumlist [users/guilds]`")
      }
    } catch (e) {
